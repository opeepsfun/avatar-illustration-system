...
const drawTop = (option: keyof typeof tops, color?: ColorProps) => {
  ...
  if (option === "Turban") {
    return `<g transform="translate(3.5 0)">
            ${svg.draw(color?.outlineColor, color?.skinColor)}
        </g>`;
  }
  ...
  return svg.draw(color?.outlineColor, color?.skinColor);
};
...
const drawGlasses = (
  option: keyof typeof eyeglasses | undefined,
  color?: ColorProps
) => {
  if (!option) return "";
...
const Avatar = ({
  ...
  top = "Fonze",
  size,
  color = { outlineColor: "#171921", skinColor: "#AC6651" },
}: AvatarProps) => {
  ...
  const baseAvatar = `
        <g id="avatar">
            ...
            <g id="eyebrow" transform="translate(120 122)">
                ${selectedEyebrow.draw(color.outlineColor, color.skinColor)}
            </g>
            ...
            <g id="eye" transform="translate(152 139)">
                ...
                ${selectedEye.draw(color.outlineColor, color.skinColor)}
            </g>
            <g id="nose" transform="translate(196.37 168.42)">
                ${selectedNose.draw(color.outlineColor, color.skinColor)}
            </g>
            <g id="ear" transform="translate(91 176.5)">
                ${earSvg}
            </g>
            <g id="shirt" transform="translate(48 292)">
                ${selectedShirt.draw(color.outlineColor, color.skinColor)}
            </g>
        </g>   
    `;
